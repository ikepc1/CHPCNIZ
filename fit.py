...
from tyro_fit import tyro, TyroFit, has_core
...
from config import COUNTER_NO, NAMES, COUNTER_POSITIONS, WAVEFORM_SIZE, TRIGGER_POSITION, NICHE_TIMEBIN_SIZE, COUNTER_QE, COUNTER_FADC_PER_PE, MC_DF_PATH
from counter_config import CounterConfig
from trigger import TriggerSim, rawphotons2fadc
from noise import read_noise_file

def E(Nmax: float | np.ndarray) -> float | np.ndarray:
...
#     @cached_property
#     def starti_dict(self) -> dict[str,int]:
#         return {f.name:int(f.waveform.argmax() - start_rise(f)) for f in self.nfits}
    
#     @cached_property
#     def endi_dict(self) -> dict[str,int]:
#         return {f.name:int(end_fall(f) - f.waveform.argmax()) for f in self.nfits}
...
#             pas.append((f.waveform-f.baseline)[im - self.starti_dict[f.name]:im + self.endi_dict[f.name]].sum())
#         return np.array(pas)
...
#         stacked_errs = []
#         for f in self.nfits:
#             n_in_sum = self.endi_dict[f.name]+self.starti_dict[f.name]
#             stacked_errs.append(np.sqrt(n_in_sum * f.baseline_error**2))
...
#         sigdict, times = ckv_signal_dict(ckv, parameters[-1])
#         pas = []
#         for name in self.nfit_dict:
#             im = sigdict[name].argmax()
#             pas.append(sigdict[name][im - self.starti_dict[name]:im + self.endi_dict[name]].sum())
#         pas = np.array(pas)
#         return pas
...
class AllSamples(FitFeature):
    ...
    @cached_property
    def real_values(self) -> np.ndarray:
        return np.hstack([(f.waveform - f.baseline)[start_rise(f):end_fall(f)] for f in self.nfits])
    
    def calc_sample_errors(self, f: NicheFit) -> np.ndarray:
        wf = (f.waveform - f.baseline)[start_rise(f):end_fall(f)]
        wf[wf<0.] = 0.
    ...
    def error(self) -> np.ndarray:
        ...
        return np.hstack([np.full(end_fall(f) - start_rise(f),f.baseline_error) for f in self.nfits])
...
class Samples(FitFeature):
    ...
    def real_values(self) -> np.ndarray:
        ...
        for f in self.nfits:
            im = f.waveform.argmax()
            stacked_wfs.extend((f.waveform-f.baseline)[im - self.starti_dict[f.name]:im + self.endi_dict[f.name]].tolist())
        return np.array(stacked_wfs)
    ...
    def get_output(self, parameters: np.ndarray) -> np.ndarray:
        ...
        for name in self.nfit_dict:
            im = sigdict[name].argmax()
            wf = sigdict[name][im - self.starti_dict[name]:im + self.endi_dict[name]]
            wf[wf>self.nfit_dict[name].max_value] = self.nfit_dict[name].max_value
            stacked_wfs.extend(wf.tolist())
        wfs = np.array(stacked_wfs)
        wfs[wfs<0.] = 0.
        return wfs
...
def dataframe_fit(df: pd.DataFrame, min_multiplicity: int = 6) -> pd.DataFrame:
    ...
    for ev_no, row in df.iterrows():
        ...
        #if no triggers go to next
        if not row['Fit']:
            dictlist.append(new_row)
            continue

        #if the event doesn't have a contained core, don't fit
        if not has_core(row['Fit']):
            dictlist.append(new_row)
            continue
        ...
        if len(nfits) >= min_multiplicity:
            ...
            new_row['chi2'] = fp.chi2ndof
            dictlist.append(new_row)
        else:
            dictlist.append(new_row)

    #make dataframe from rows
    fit_df = pd.DataFrame(dictlist)
...
if __name__ == '__main__':
    ...
    all_mc_df_paths = [p for p in mc_df_paths if p.name.startswith('mc')]
    already_fit_paths = [p for p in mc_df_paths if p.name.startswith('fit')]
